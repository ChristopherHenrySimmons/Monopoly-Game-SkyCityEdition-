...
namespace Monopoly
{
    ...
    public class Monopoly : Game
    {
        ...
        public override void makePlay(int iPlayerIndex)
        {
            ...
            //ADDED if statment to give the system the ablity to check if player has landed on the Casino Skycity
            if (player.IsInCasino)
            {
                this.displayMenuInCasino(iPlayerIndex);
            }
        ...
        public void setUpPlayers()
        {
            ...
            for (int i = 0; i < playerCount; i++)
            {
                ...
                //New casino event
                player.playerGoCasino += playerGoCasinoHandler;
        ...
        public void displayPlayerChoiceMenu(Player player)
        {
            ...
            Console.WriteLine("                 13. Change Rent of this Property");
            Console.WriteLine("                 =======================================");
            Console.Write("                 (1-13)=>");
            //read response
            resp = inputInteger();
        ...
        {
            string sPrompt = String.Format("{0}Please select a property to change rent:", this.playerPrompt(player));
            ...
            //check that it is a residential should remove ???
            ...
            {
                Console.WriteLine("---------------------------------------------------------------------------");
                Console.WriteLine(">>>>>>>>>>{0} rent cannot be changed for {1} because it is not a Residential Property.<<<<<<<<<<", this.playerPrompt(player), property.getName());
                return;
            }

            Console.WriteLine("-----------------------------------------------------------");
            bool doChangeRent = this.getInputYN(player, String.Format("You chose to change rent for a house on {0}. Are you sure?", residential.getName()));
            //if confirmed
            if (doChangeRent)
            {
                decimal oldRent = residential.getRent();
                Console.Write("Enter new rent: ");
                decimal newRent = Convert.ToDecimal(Console.ReadLine());
                residential.setRent(newRent);
        ...
        public void buyHotel(Player player)
        {
            ...
            //get the property to buy hotel for
            Property property = this.displayPropertyChooser(player.getPropertiesOwnedFromBoard(), sPrompt);
            //if dont own any properties
            ...
            //check that max hotel has not been reached
        ...
        //Goto handler for Casino
        public static void playerGoCasinoHandler(object obj, EventArgs args)
        {
            int indexOfCasino = -1;
            Player p = (Player)obj;
            for (int i = 0; i < Board.access().getProperties().Count; i++)
            {
                if ("SkyCity" == ((Property)(Board.access().getProperties()[i])).getName())
                {
                    indexOfCasino = i;
                    break;
                }
            ...
            {
                Console.WriteLine("-----------------------------------------------");
                Console.WriteLine(">>>>>>>>>>Error! Can not find SkyCity!!!<<<<<<<<<<");
                return;
            }

            p.setLocation(indexOfCasino);
            p.IsInCasino = true;
            p.DoubleCount = 0;
        }
        //End
        ...
        public void loadGame()
        {
            ...
            if (!loadData())
            {
                Console.WriteLine(">>>>>>>>>>Load save file failure!<<<<<<<<<<");
                return;
            }
            ...
            for (int i = initIndexOfPlayer; i < totalAmtOfPlayers; i++)
            {
                ...
                else
                {
                    ...
                    if (i != initIndexOfPlayer)
                    {
                        ...
                        //Msg the player to inform them there in SkyCity
                        if (player.IsInCasino)
                        {
                            Console.WriteLine("---------------------------------------------------------------------");
                            Console.WriteLine($">>>>>>>>>>{player.getName()}: You are in SkyCity now<<<<<<<<<<");
                            Console.WriteLine("Press any key to continue!!!==>");
                            Console.ReadLine();
                        }
                    }                  
        ...
        public void displayMenuInCasino(int indexOfPlayer)
        {
            ...
            Console.WriteLine("                  You landed on SkyCity:");

            Console.WriteLine("-----------------------------------------------------------");
            bool gamble = this.getInputYN(player, String.Format("Do you want to gamble at SkyCity Y/N?"));
            //if confirmed
            if (gamble)
            {
                Console.WriteLine(">>>>>>>>>>{0} you got a problem<<<<<<<<<<", playerPrompt(player));
                Console.WriteLine("                 ---------------------------------------");
                Console.WriteLine("                  1. Finish turn");
                Console.WriteLine("                  2. View your details");
                Console.WriteLine("                  3. Gamble double roll (Roll a double to Win) (Cost:$50 big ones)");
                Console.WriteLine("                  4. Gamble Single roll (Roll above 3 to Win) (Cost:$50 big ones)");
                ...
                switch (resp)
                {
                    ...
                    case 3:
                        ...
                        {
                            Console.WriteLine("---------------------------------------------------------------------------");
                            Console.WriteLine($">>>>>>>>>>{player.getName()}: You have failed to roll double thrice, they kicked you out!!!<<<<<<<<<<");
                            this.makePlay(indexOfPlayer);
                        }
                        else
                        {
                            player.pay(50);
                            this.gambleDouble(player);
                        }
                        if (player.DoubleCount == 1)
                        {
                            player.receive(500);
                    ...
                    case 4:
                        ...
                        {
                            Console.WriteLine("---------------------------------------------------------------------------");
                            Console.WriteLine($">>>>>>>>>>{player.getName()}: You have failed to roll above 3 thrice, they kicked you out!!!<<<<<<<<<<");
                            this.makePlay(indexOfPlayer);
                        }
                        else
                            this.gambleSingle(player);
                        break;
                    case 5:
                        this.saveGame();
                        this.displayMenuInCasino(indexOfPlayer);
    ...
    public void displayMenuInJail(int indexOfPlayer)
        ...
        //Roll double for SkyCity
        public bool gambleDouble(Player player)
        {
            int i, j;
            Dice dice = new Dice();
            i = j = 0;
        ...
        //Roll single for SkyCity
        public bool gambleSingle(Player player)
        {
            int i;
            Dice dice = new Dice();
            i = 0;
            ...
            if (i >3)
            {
                ...
                player.setLocation(player.getLocation() + i);
                Property propertyLandedOn = Board.access().getProperty(player.getLocation());
                Console.WriteLine(">>>>>>>>>>" + propertyLandedOn.landOn(ref player) + "<<<<<<<<<<");
                Console.WriteLine("\n{0}:\t{1}", player.getName(), player.BriefDetailsToString());
            ...
            {
                Console.WriteLine("----------------------------------------------------------------------------");
                Console.WriteLine($">>>>>>>>>>{player.getName()}:It's a pity you fail to roll above 3 and your turn is over!!!<<<<<<<<<<");
                player.FailureRollDouble++;
                Console.WriteLine("Press any key to continue!!!==>");
                Console.ReadLine();
                return false;
